    ...
    <div v-if="chamados.register.length > 0" class="relative">
        <div  class="w-full h-auto p-4 rounded-md bg-slate-300 dark:bg-gray-900 dark:text-white flex flex-wrap items-center justify-center gap-4">
            ...
            <div :class="{'concluido': chamado.concluido}" v-for="(chamado, index) in chamados.register" :key="index" class="relative ativo w-full max-w-xs p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-4 dark:bg-gray-800 dark:border-gray-700">
                ...
                <button v-if="!chamado.concluido" @click="abrirEditor(index), openModal()"  class="absolute inline-flex items-center justify-center w-5 h-5 text-xs font-bold hover:bg-blue-700 dark:text-white dark:border-gray-300 border-gray-500 text-black border rounded-md top-1 right-16 "><span class="material-symbols-outlined text-sm">edit</span></button>
...
<!-- Main modal -->
<div v-if="modal == true" class="fixed top-0 left-0 z-50 right-0 flex items-center justify-center bg-opacity-60 bg-gray-950 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full">
    <div class="relative w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="authentication-modal">
                <span @click="cancelarEdicao, doneModal()"><span class="material-symbols-outlined">close</span></span>
            </button>
